  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        ...
        <input type="text" inputMode="decimal" value={formatInputValue(value)} onChange={(event) => { const rawValue = event.target.value.replace(/,/g, ''); onChange(rawValue === '' ? '' : Math.max(0, Number(rawValue) || 0)); }} className="w-full pl-10 pr-4 py-3.5 bg-white/10 border border-white/20 rounded-2xl text-white font-bold focus:outline-none focus:border-amber-400 transition-colors" />
      </div>
    </label>
  );
}
